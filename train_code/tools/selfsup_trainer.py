...
from data.widerface_transfer import WIDERDetectionTransfer
...
class SSSingleDataloader():
	def __init__(self, batch_size, num_workers, file, ss_type, test=False):
		if ss_type == 'contrastive':
			ss_dataset = WIDERDetectionContrastive(file)
		elif ss_type == 'transfer':
			ss_dataset = WIDERDetectionTransfer(file, test)
		else:
			exit('Unknown self-supervised learning type %s'%(ss_type))
