...
class DSFD(nn.Module):
    ...
    def forward(self, x, detection=True):
        size = x.size()[2:]
        pal1_sources = list()
        pal2_sources = list()
        loc_pal1 = list()
        ...
        self.priors_pal1 = Variable(priorbox.forward(), volatile=True).to(x.device)

        priorbox = PriorBox(size, features_maps, cfg, pal=2)
        self.priors_pal2 = Variable(priorbox.forward(), volatile=True).to(x.device)
...
def multibox(vgg, extra_layers, num_classes):
    ...
    for k, v in enumerate(vgg_source):
        feat_out_channels.append(vgg[v].out_channels)
        loc_layers += [nn.Conv2d(vgg[v].out_channels,
                                 4, kernel_size=3, padding=1)]
        conf_layers += [nn.Conv2d(vgg[v].out_channels,
                                  num_classes, kernel_size=3, padding=1)]
    for k, v in enumerate(extra_layers[1::2], 2):
        feat_out_channels.append(v.out_channels)
        loc_layers += [nn.Conv2d(v.out_channels,
                                 4, kernel_size=3, padding=1)]
        conf_layers += [nn.Conv2d(v.out_channels,
                                  num_classes, kernel_size=3, padding=1)]
    return (loc_layers, conf_layers), feat_out_channels


def build_net_vgg(phase, num_classes=2):
    base = vgg(vgg_cfg, 3)
    extras = add_extras(extras_cfg, 1024)
    head1, _ = multibox(base, extras, num_classes)
    head2, feat_out_channels = multibox(base, extras, num_classes)
    fem = fem_module(fem_cfg)
    return DSFD(phase, base, extras, fem, head1, head2, num_classes), feat_out_channels
